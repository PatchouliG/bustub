...
int B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(const KeyType &key, const KeyComparator &comparator) const {
  for (auto i = 0; i < GetSize(); ++i) {
    ...
    if (comparator(kv.first, key) >= 0) return i;
  }
  //  not found
  return -1;
}  // namespace bustub
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &comparator) {
  auto position = KeyIndex(key, comparator);
  if (position != -1) {
    std::memmove(array + position + 1, array + position, (GetSize() - position) * sizeof(array[0]));
  } else {
    //    insert at end
    position = GetSize();
  }
  array[position] = std::pair<KeyType, ValueType>(key, value);
  SetSize(GetSize() + 1);
  return GetSize();
}
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveLastToFrontOf(BPlusTreeLeafPage *recipient) {
  memmove(recipient->array + 1, array, GetSize() * sizeof(MappingType));
